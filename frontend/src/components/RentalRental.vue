<template>
    <v-card style="width:450px; height:100%;" outlined>
        ...
        <v-card-title v-if="value._links">
            Rental # {{decode(value._links.self.href.split("/")[value._links.self.href.split("/").length - 1])}}
        </v-card-title >
        <v-card-title v-else>
            Rental
        </v-card-title >        
        ...
        <v-card-actions>
            ...
            <v-btn
                ...
                color="primary"
                text
                @click="openReturnBook"
            >
                ReturnBook
            </v-btn>
            <v-dialog v-model="returnBookDiagram" width="500">
                <ReturnBookCommand
                    @closeDialog="closeReturnBook"
	...
	async created() {
        ...
        methods: {
            ...
            async rentBook() {
                try {
                    if(!this.offline){
                        var temp = await axios.post(axios.fixUrl(this.value._links['/rentbook'].href))
                        for(var k in temp.data) this.value[k]=temp.data[k];
                    }
            ...
            async returnBook(params) {
                try {
                    if(!this.offline) {
                        var temp = await axios.put(axios.fixUrl(this.value._links['returnbook'].href), params)
